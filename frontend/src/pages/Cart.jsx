...
export const Cart = () => {
    ...
    const handleClearCart = async () => {
        ...
        const response = await axios.delete('https://tourism-website-server-tau.vercel.app/user/deleteallcart', {
            headers: {
            ...
            {cart && cart.length > 0 ? (
                <>
                    <section className='col-12 col-md-6 d-flex flex-column justify-content-center align-items-center'>
                        {cart.map((item) => (
                            <Cartitem key={item.id} item={item} handleItemDelete={handleItemDelete}
